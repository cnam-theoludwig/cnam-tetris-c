...
SDL_LIB = -L${SDL_BUILD_FOLDER} -Wl,-rpath,${SDL_BUILD_FOLDER} -lSDL2 -ldl -lpthread
...
SDL_TTF_LIB = -L${SDL_TTF_BUILD_FOLDER} -Wl,-rpath,${SDL_TTF_BUILD_FOLDER} -lSDL2_ttf
...
SDL_IMAGE_LIB = -L${SDL_IMAGE_BUILD_FOLDER} -Wl,-rpath,${SDL_IMAGE_BUILD_FOLDER} -lSDL2_image
...
${SDL_PATH}: ${DEPENDENCIES_FOLDER}
	if [ ! -d "${SDL_FOLDER}" ]; then \
		...
		cd ${SDL_BUILD_FOLDER} && cmake .. -DCMAKE_INSTALL_PREFIX=${SDL_BUILD_FOLDER} && $(MAKE) -j$(nproc) && $(MAKE) install; \
	fi

${SDL_IMAGE_PATH}: ${SDL_PATH}
	if [ ! -d "${SDL_IMAGE_FOLDER}" ]; then \
		...
		cd ${SDL_IMAGE_BUILD_FOLDER} && cmake .. -DCMAKE_INSTALL_PREFIX=${SDL_IMAGE_BUILD_FOLDER} -DSDL2_DIR=${SDL_BUILD_FOLDER} -DSDL2_IMAGE_SAMPLES=OFF -DSDL2_IMAGE_WEBP=OFF -DSDL2_IMAGE_AVIF=OFF && $(MAKE) -j$(nproc) && $(MAKE) install; \
	fi
...
${SDL_TTF_PATH}: ${SDL_PATH} ${SDL_IMAGE_PATH}
	if [ ! -d "${SDL_TTF_FOLDER}" ]; then \
		...
		cd ${SDL_TTF_BUILD_FOLDER} && cmake .. -DCMAKE_INSTALL_PREFIX=${SDL_TTF_BUILD_FOLDER} -DSDL2_DIR=${SDL_BUILD_FOLDER} && $(MAKE) -j$(nproc) && $(MAKE) install; \
	fi
