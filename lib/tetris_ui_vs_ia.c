...
static void ia_compute_best_move(struct Tetris* ia, int *best_rot, int *best_col) {
    ...
    for (int rot = 0; rot < 4; ++rot) {
        for (int col = 0; col < GRID_WIDTH; ++col) {
            ...
            while (dx > 0) { tetris_last_tetromino_step_right(copy); dx--; }
            while (dx < 0) { tetris_last_tetromino_step_left(copy);  dx++; }
...
TetrisUIAction tetris_ui_vs_ai(struct Tetris* human,
    ...
    if (window_in && *window_in && renderer_in && *renderer_in) {
        ...
        if (!renderer) { SDL_DestroyWindow(window); return UI_ACTION_QUIT; }
        own_window = true;
        *window_in   = window;
            ...
            if (!paused) {
                if (event.type == SDL_MOUSEBUTTONDOWN && event.button.button == SDL_BUTTON_LEFT) {
                    ...
                    switch (event.key.keysym.sym) {
                        case SDLK_LEFT:  tetris_last_tetromino_step_left(human);  break;
                        case SDLK_RIGHT: tetris_last_tetromino_step_right(human); break;
                        case SDLK_DOWN:  tetris_last_tetromino_step_down(human);  break;
                        case SDLK_r:     tetris_last_tetromino_step_rotate_right(human); break;
                        case SDLK_e:     tetris_last_tetromino_step_rotate_left(human);  break;
                        case SDLK_SPACE:
